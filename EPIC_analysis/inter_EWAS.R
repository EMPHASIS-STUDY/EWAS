...
#All
####
design_all <- model.matrix(~ Slide + Array + Bcell + CD4T + CD8T + Eos +
                           Mono + NK + Neu + Sex + Age + MooreSoC +
                           MasterGroupNo, pdata)
...
#write results
write.csv(as.data.frame(res_DMPs_all)[1:100,],
          file="../results/EPIC_EWAS_top_100_DMPs_all.csv")
...
design_all_no_BCC <-  model.matrix(~ Slide + Array + Sex + Age +
                                     MooreSoC + MasterGroupNo, pdata)

DMPs_all_no_BCC <- lmFit(norm_mval_fil,design_all_no_BCC)
DMPs_all_no_BCC  <- eBayes(DMPs_all_no_BCC )
res_DMPs_all_no_BCC <- topTable(DMPs_all_no_BCC,
                                coef = "MasterGroupNo2", number = Inf,
                                genelist=anno_sub, sort.by="B")

write.csv(as.data.frame(res_DMPs_all_no_BCC)[1:100,],
          file="../results/EPIC_EWAS_top_100_DMPs_all_no_BCC.csv")
...
#SVs
#####
design_svs <- model.matrix(~ SV1 + SV2 + SV3 + SV4 + SV5 + SV6 + SV7 + SV8 +
                             SV9 + SV10 + SV11 + SV12 + SV13 + SV15 + SV17 + SV19 + SV20 + Age + MooreSoC + MasterGroupNo, cbind(pdata,svs))
...
#write results
write.csv(as.data.frame(res_DMPs_svs)[1:100,],
          file="../results/EPIC_EWAS_top_100_DMPs_svs.csv")
...
#PCs
#########
design_pcs <- model.matrix(~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 +
                           PC8 + PC9 + PC10 + PC11 + PC12 + PC13 + PC15 +
                           Age + MooreSoC + MasterGroupNo, cbind(pdata,pcs))
...
#write results
write.csv(as.data.frame(res_DMPs_pcs)[1:100,],
          file="../results/EPIC_EWAS_top_100_DMPs_pcs.csv")
...
res_DMPs_pcs_deltab <- add_delta_B(as.data.frame(res_DMPs_pcs)[1:100,],
                                  norm_beta_fil,pdata$MasterGroupNo)
write.csv(res_DMPs_pcs_deltab,
          file="../results/EPIC_EWAS_top_100_DMPs_pcs_deltab.csv")
...
#######################################
design_pcs_inter <- model.matrix(~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 +
                                 PC7 + PC8 + PC9 + PC10 + PC11 + PC12 +
                                 PC13 + PC15 + Age + MooreSoC * MasterGroupNo, cbind(pdata,pcs))

DMPs_pcs_inter <- lmFit(norm_mval_fil,design_pcs_inter)
DMPs_pcs_inter <- eBayes(DMPs_pcs_inter)
res_DMPs_pcs_inter <- topTable(DMPs_pcs_inter,
                               coef = "MooreSoCrainy:MasterGroupNo2",
                               number = Inf, genelist=anno_sub, sort.by="B")

inter_main_group <- topTable(DMPs_pcs_inter,
                             coef = "MasterGroupNo2", number = Inf,
                             genelist=anno_sub, sort.by="B")
inter_main_season <- topTable(DMPs_pcs_inter,
                              coef = "MooreSoCrainy", number = Inf,
                              genelist=anno_sub, sort.by="B")
...
#GO
###
gst_all <- gometh(sig.cpg=res_DMPs_all$Name[1:1000],
                  collection="GO",array.type="EPIC")
gst_all_no_bcc <- gometh(sig.cpg=res_DMPs_all_no_BCC$Name[1:1000],
                         collection="GO",array.type="EPIC")
gst_svs <- gometh(sig.cpg=res_DMPs_svs$Name[1:1000],
                  collection="GO",array.type="EPIC")
gst_isvs <- gometh(sig.cpg=res_DMPs_isvs$Name[1:1000],
                    collection="GO",array.type="EPIC")
gst_pcs <- gometh(sig.cpg=res_DMPs_pcs$Name[1:1000],
                  collection="GO",array.type="EPIC")

#write results
write.csv(topGO(gst_all),file="../results/EPIC_EWAS_GO_pathways_all.csv")
write.csv(topGO(gst_all_no_bcc),
          file="../results/EPIC_EWAS_GO_pathways_all_no_BCC.csv")
...
#KEGG
#####
gst_all_KEGG <- gometh(sig.cpg=res_DMPs_all$Name[1:1000],
                      collection="KEGG",array.type="EPIC")
gst_all_no_bcc_KEGG <- gometh(sig.cpg=res_DMPs_all_no_BCC$Name[1:1000],
                              collection="KEGG",array.type="EPIC")
gst_svs_KEGG <- gometh(sig.cpg=res_DMPs_svs$Name[1:1000],
                        collection="KEGG",array.type="EPIC")
gst_isvs_KEGG <- gometh(sig.cpg=res_DMPs_isvs$Name[1:1000],
                        collection="KEGG",array.type="EPIC")
gst_pcs_KEGG <- gometh(sig.cpg=res_DMPs_pcs$Name[1:1000],
                        collection="KEGG",array.type="EPIC")

#write results
write.csv(topKEGG(gst_all_KEGG),
          file="../results/EPIC_EWAS_KEGG_pathways_all.csv")
write.csv(topKEGG(gst_all_no_bcc_KEGG),
          file="../results/EPIC_EWAS_KEGG_pathways_all_no_BCC.csv")
write.csv(topKEGG(gst_svs_KEGG),
          file="../results/EPIC_EWAS_KEGG_pathways_SVs.csv")
write.csv(topKEGG(gst_isvs_KEGG),
          file="../results/EPIC_EWAS_KEGG_pathways_ISVs.csv")
write.csv(topKEGG(gst_pcs_KEGG),
          file="../results/EPIC_EWAS_KEGG_pathways_PCs.csv")
...
#DMRcate
#########
DMRs_pcs<- fastEPICdmrcate(norm_mval_fil,design_pcs,
                           coef="MasterGroupNo2",fdr=0.05,
                           pcutoff=5e-2, mc.cores=8)
DMRs_svs <- fastEPICdmrcate(norm_mval_fil,design_svs,
                            coef="MasterGroupNo2",fdr=0.05,
                            pcutoff=5e-2, mc.cores=8)
DMRs_isvs <- fastEPICdmrcate(norm_mval_fil,design_isvs,
                             coef="MasterGroupNo2",fdr=0.05,
                             pcutoff=5e-2, mc.cores=8)
...
DMR.plot(ranges=DMRs_pcs_annot, dmr=1,
          CpGs=norm_beta_fil, what="Beta", arraytype = "EPIC",
         phen.col=cols, genome="hg19")
DMR.plot(ranges=DMRs_pcs_annot, dmr=277,
          CpGs=norm_beta_fil, what="Beta", arraytype = "EPIC",
         phen.col=cols, genome="hg19")
DMR.plot(ranges=DMRs_pcs_annot, dmr=2,
          CpGs=norm_beta_fil, what="Beta", arraytype = "EPIC",
         phen.col=cols, genome="hg19")
